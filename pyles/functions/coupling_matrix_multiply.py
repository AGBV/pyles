...
from .coupling_matrix.cpu.numba_parallel import compute_idx_lookups
from .coupling_matrix.cpu.numba_parallel import particle_interaction
from .coupling_matrix.gpu.numba_parallel import particle_interaction_gpu
...
def coupling_matrix_multiply_numba(simulation, x: np.ndarray, k_medium: complex=None):
  ...
  wx = np.zeros(x.shape + (wavelengths,), dtype=complex)
  ...
  if simulation.numerics.gpu:
    ...
    # blocks_per_grid_x = (jmax         + threads_per_block[0] - 1) // threads_per_block[0]
    # blocks_per_grid_y = (jmax         + threads_per_block[1] - 1) // threads_per_block[1]
    # blocks_per_grid_z = (wavelengths  + threads_per_block[2] - 1) // threads_per_block[2]
