...
class Optics:
  ...
  def compute_cross_sections(self):
    ...
    self.c_ext = np.zeros(self.simulation.parameters.wavelengths_number, dtype=complex)
    self.c_ext -= np.sum(np.conjugate(a) * f, axis=(0,1)) / np.power(self.simulation.parameters.k_medium, 2) * np.pi
    ...
    self.c_sca = c_sca / np.power(self.simulation.parameters.k_medium, 2) * np.pi
  ...
  def compute_phase_funcition(self, legendre_coefficients_number: int=15, c_and_b: Union[bool, tuple]=False):
    pilm, taulm = spherical_functions_trigon(self.simulation.numerics.polar_angles, self.simulation.numerics.lmax)
    ...
    if self.simulation.numerics.gpu:
      ...
      e_1_sca_real = np.zeros((self.simulation.numerics.azimuthal_angles.size, 3, self.simulation.parameters.k_medium.size), dtype=float)
      e_1_sca_imag = np.zeros_like(e_1_sca_real)

      particles_position          = cuda.to_device(self.simulation.parameters.particles.pos)
      ...
      azimuthal_angles_device     = cuda.to_device(self.simulation.numerics.azimuthal_angles)
    ...
    self.phase_function = np.sum(np.power(np.abs(e_1_sca), 2), axis=1) * 4 * np.pi / np.power(self.simulation.parameters.k_medium, 2) / self.c_sca[np.newaxis,:]
    self.phase_function_legendre_coefficients = np.polynomial.legendre.legfit(np.cos(self.scattering_angles), self.phase_function, legendre_coefficients_number)
    ...
    if (self.simulation.numerics.sampling_points_number is not None) and (self.simulation.numerics.sampling_points_number.size == 2):
      ...
      self.scattering_angles = np.reshape(self.scattering_angles, self.simulation.numerics.sampling_points_number)
  ...
  def compute_double_henyey_greenstein(theta: np.ndarray, cb: np.ndarray):
    ...
    p1 = (1-cb[1]**2) / np.power(1 - 2 * cb[1] * np.cos(theta) + cb[1]**2, 3/2)
    p2 = (1-cb[2]**2) / np.power(1 + 2 * cb[2] * np.cos(theta) + cb[2]**2, 3/2)
  ...
  def __compute_c_and_b(self):
    ...
    for w in range(self.phase_function.shape[1]):
      dhg_optimization = lambda bc: (Optics.compute_double_henyey_greenstein(self.scattering_angles, bc) - self.phase_function[:,w])
      bc = least_squares(dhg_optimization, bc0, jac='2-point', bounds=self.c_and_b_bounds)
      self.cb[w, :] = bc.x