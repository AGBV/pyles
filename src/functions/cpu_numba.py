...
def particle_interaction(lmax: int, particle_number: int, idx: np.ndarray, x: np.ndarray, translation_table: np.ndarray, plm: np.ndarray, sph_h: np.ndarray, e_j_dm_phi, progress_proxy=None):
    ...
    wx = np.zeros(x.size * channels,
                  dtype=complex128).reshape(x.shape + (channels,))
...
def compute_idx_lookups(lmax: int, particle_number: int):
    nmax = 2 * lmax * (lmax + 2)
    idx = np.zeros(nmax * particle_number * 5,
                   dtype=int64).reshape((nmax * particle_number, 5))
    ...
    for j_idx in prange(jmax * jmax):
        ...
        for p in range(delta_m, 2 * lmax + 1):
            p_dependent += (translation_table[n2, n1, p] *
                            plm[p * (p + 1) // 2 + delta_m, s1, s2] *
                            sph_h[p, s1, s2, :])
        p_dependent *= np.conj(sfc[s1, n1, :]) * \
            e_j_dm_phi[m2 - m1 + 2 * lmax, s1, s2] * sfc[s2, n2, :]
...
def compute_radial_independent_scattered_field(lmax: int, particles_position: np.ndarray, idx: np.ndarray, sfc: np.ndarray, k_medium: np.ndarray, azimuthal_angles: np.ndarray, e_r: np.ndarray, e_phi: np.ndarray, e_theta: np.ndarray, pilm: np.ndarray, taulm: np.ndarray, progress_proxy=None):
    ...
    e_1_sca = np.zeros(azimuthal_angles.size * 3 * k_medium.size,
                       dtype=complex128).reshape((azimuthal_angles.size, 3, k_medium.size))
    ...
    for global_idx in prange(jmax * azimuthal_angles.size * k_medium.size):
        ...
        if tau == 1:
            e_1_sca[a, :, w] += t * (e_theta[a, :] * pilm[l,  np.abs(m), a]
                                     * 1j * m - e_phi[a, :] * taulm[l, np.abs(m), a])
        else:
            e_1_sca[a, :, w] += t * (e_phi[a, :] * pilm[l,  np.abs(m), a]
                                     * 1j * m + e_theta[a, :] * taulm[l, np.abs(m), a])
