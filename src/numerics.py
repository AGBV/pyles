...
class Numerics:
  def __init__(self, lmax: int, sampling_points_number: Union[int, np.ndarray]=100, polar_angles: np.ndarray=None, azimuthal_angles: np.ndarray=None, gpu: bool=False, particle_distance_resolution = 10.0, solver=None):
    ...
    if (polar_angles is None) or (azimuthal_angles is None):
      if self.sampling_points_number.size == 0:
        ...
        self.log.warning('Number of sampling points cant be an empty array. Reverting to 100 points (Fibonacci sphere).')
      elif self.sampling_points_number.size > 2:
        self.sampling_points_number = np.array([sampling_points_number[0]])
        self.log.warning('Number of sampling points with more than two dimensions is not supported. Reverting to the first element in the provided array (Fibonacci sphere).')
      
      if self.sampling_points_number.size == 1:
        _, polar_angles, azimuthal_angles = Numerics.compute_fibonacci_sphere_points(sampling_points_number[0])
      elif self.sampling_points_number.size == 2:
        self.polar_angles_linspace = np.linspace(0, np.pi, sampling_points_number[1])
        self.azimuthal_angles_linspace = np.linspace(0, 2 * np.pi, sampling_points_number[0] + 1)[:-1]
