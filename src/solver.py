    ...
    def run(self, a: LinearOperator, b: np.ndarray, x0: np.ndarray = None):
        ...
        if self.type == 'bicgstab':
            ...
            value, err_code = bicgstab(
                a, b, x0, tol=self.tolerance, atol=0, maxiter=self.max_iter, callback=counter)
        elif self.type == 'gmres':
            counter = GMResCounter(callback_type='pr_norm')
            value, err_code = gmres(a, b, x0, restart=self.restart, tol=self.tolerance, atol=self.tolerance **
                                    2, maxiter=self.max_iter, callback=counter, callback_type='pr_norm')
