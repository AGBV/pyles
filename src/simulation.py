...
class Simulation:
    ...
    def legacy_compute_h3_table(self):
        ...
        at the points calculated in [Simulation.legacy_compute_lookup_particle_distances()][src.simulation.Simulation.legacy_compute_lookup_particle_distances].
        ...
        self.h3_table = np.zeros(
            (2 * self.numerics.lmax + 1,
             self.lookup_particle_distances.shape[0], self.parameters.medium_mefractive_index.shape[0]),
            dtype=complex)
        size_param = np.outer(
    ...
    def __compute_idx_lookup(self):
        ...
        The lookup table is created using [compute_idx_lookups][src.functions.cpu_numba.compute_idx_lookups].
    ...
    def __compute_lookups(self):
        ...
        ct = np.divide(
            np.subtract.outer(
                self.parameters.particles.pos[:, 2], self.parameters.particles.pos[:, 2]),
            dists,
            out=np.zeros((particle_number, particle_number)),
            where=dists != 0)
        phi = np.arctan2(
            np.subtract.outer(
                self.parameters.particles.pos[:, 1], self.parameters.particles.pos[:, 1]),
        ...
        self.sph_h = np.zeros((2 * lmax + 1, particle_number, particle_number,
                              self.parameters.k_medium.shape[0]), dtype=complex)
        self.sph_j = np.zeros_like(self.sph_h)
        self.e_j_dm_phi = np.zeros(
            (4 * lmax + 1, particle_number, particle_number), dtype=complex)
        self.plm = np.zeros(((lmax + 1) * (2 * lmax + 1),
                            particle_number, particle_number))

        for p in range(2 * lmax + 1):
            self.sph_h[p, :, :, :] = np.sqrt(np.divide(np.pi / 2, size_param, out=np.zeros_like(
                size_param), where=size_param != 0)) * hankel1(p + 1/2, size_param)
        ...
        self.log.info('Computing lookup tables took %f s' % (
    ...
    def __setup(self):
        ...
        - [__compute_idx_lookups][src.simulation.Simulation.__compute_idx_lookup]
        - [__compute_lookups][src.simulation.Simulation.__compute_lookups]
        """
    ...
    def compute_mie_coefficients(self):
        ...
        See Also
        --------
        [t_entry][src.functions.t_entry.t_entry] : T-Matrix entry function
    ...
    def __compute_initial_field_coefficients_planewave(self):
        ...
        relative_particle_positions = self.parameters.particles.pos - \
        ...
        for m in range(-lmax, lmax+1):
            for tau in range(1, 3):
                for l in range(np.max([1, np.abs(m)]), lmax+1):
                    ...
                    self.initial_field_coefficients[:, n, :] = 4 * E0 * np.exp(-1j * m * alpha) * eikr * transformation_coefficients(
    ...
    def coupling_matrix_multiply(self, x: np.ndarray, idx: int = None):
        ...
        if self.numerics.gpu:
            ...
            time_end = time()
            self.log.info("\t Time taken for preparation: %f" %
                          (coupling_matrix_time - preparation_time))
            self.log.info("\t Time taken for coupling matrix: %f" %
    ...
    def master_matrix_multiply(self, value: np.ndarray, idx: int):
        ...
        t_matrix_stop = time()
        self.log.info('\t done in %f seconds.' %
                      (t_matrix_stop - t_matrix_start))
    ...
    def compute_scattered_field_coefficients(self, guess=None):
        ...
        self.scattered_field_coefficients = np.zeros_like(
            self.initial_field_coefficients)
        self.scattered_field_err_codes = np.zeros(
            self.parameters.wavelengths_number)
        if guess is None:
        ...
        for w in range(self.parameters.wavelengths_number):
            ...
            x0 = guess[:, :, w].ravel()
            self.log.info('Solver run %d/%d' %
                          (w+1, self.parameters.wavelengths_number))
