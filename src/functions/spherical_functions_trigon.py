...
def spherical_functions_trigon(theta, lmax):
    ...
    for l in range(1, lmax):
        plm[l+1, 0, :] = 1/(l+1) * np.sqrt((2*l + 1) * (2*l + 3)) * ct * \
            plm[l, 0, :] - l/(l+1) * np.sqrt((2*l + 3) /
                                             (2*l - 1)) * plm[l-1, 0, :]
